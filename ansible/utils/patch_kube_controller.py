...
if __name__ == '__main__':
    ...
    parser.add_argument("-i", "--inplace", help="Modify the file in-place", action="store_true")
    ...
    if len([vm for vm in data["spec"]["containers"][0]["volumeMounts"] if vm["name"] == "cloud-config"]) == 0:
        data["spec"]["containers"][0]["volumeMounts"].append(volumeMount)
    if len([v for v in data["spec"]["volumes"] if v["name"] == "cloud-config"]) == 0:
