...
in {
  imports = [
    ...
    (mkRenamedOptionModule "k8s-service-layer.prometheus" "prometheus_operator_cpu_request" "operator_cpu_request")
    (mkRenamedOptionModule "k8s-service-layer.prometheus" "prometheus_operator_cpu_limit" "operator_cpu_limit")
    (mkRenamedOptionModule "k8s-service-layer.prometheus" "prometheus_operator_memory_request" "operator_memory_request")
    (mkRenamedOptionModule "k8s-service-layer.prometheus" "prometheus_operator_memory_limit" "operator_memory_limit")
  ];
  options.yk8s.k8s-service-layer.prometheus =
    mkTopSection {
      ...
      use_thanos = mkEnableOption "Enable use of Thanos";
      ...
      scheduling_key = mkOption {
        ...
        '';
        type = with types; nullOr str;
        # TODO conflicting values: role had null
        default = "node-restriction.kubernetes.io/cah-managed-k8s-monitoring";
      ...
      thanos_store_in_memory_max_size = mkOption {
        ...
        '';
        type = types.strMatching "0|([0-9]+[MG]B)";
        default = "0";
      };
      ...
      internet_probe_targets = mkOption {
        ...
        type = types.listOf (types.submodule {
          options = {
            ...
            module = mkOption {
              description = ''
                module to be used. Can be "http_2xx" (default), "icmp" or "tcp_connect".
              '';
              type = types.strMatching "http_2xx|tcp_connect";
              default = "http_2xx";
            };
          };
        });
    ...
    // mkMultiResourceOptions {
      ...
      resources = {
        ...
        grafana.cpu.request = "500m"; # TODO conflicting values: role had 100m
        ...
        thanos_query.cpu.request = "1"; # TODO conflicting values: role had 100m

        thanos_compact.memory.limit = "200Mi";
        thanos_compact.cpu.request = "100m";

        thanos_store.memory.limit = "2Gi";
        thanos_store.cpu.request = "500m"; # TODO conflicting values: role had 100m
      };
    };
