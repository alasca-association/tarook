...
variable "nodes" {
  description = <<-EOT
    ...
    'anti_affinity_group_name' must not be set when role!="worker"
    Leaving 'anti_affinity_group_name' empty means to not join any anti affinity group
  EOT
  ...
  type = map(
    object({
      ...
      root_disk_volume_type    = optional(string)
      anti_affinity_group_name = optional(string)  # only for worker nodes
    })
  )
  ...
  validation {
    condition     = alltrue([for x in var.nodes: x.anti_affinity_group_name == null if x.role == "master"])
    error_message = "'anti_affinity_group_name' must not be set for master nodes"
  }
}
