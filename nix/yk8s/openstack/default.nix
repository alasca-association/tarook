  ...
  in
    ...
    then
      throw ''
        ...
        Set `infra.cluster_name` back to ${current_config.cluster_name}. Your suggested change ${v} is unacceptable.
      ''
    else v;
  ...
  options.yk8s.openstack = mkTopSection {
    ...
    _docs.preface = ''
      ...
      - for master/worker nodes:
        ``openstack.cluster_name`` ``<the nodes' table name>``

      - for gateway nodes:
        ``openstack.cluster_name`` ``terraform.gateway_defaults.common_name`` ``<numeric-index>``
      ...
      (let
        ...
        current_cluster_name =
          ...
          then current_config.cluster_name
            ...
            "managed-k8s";
      in {
        assertion = cluster_exists -> (config.yk8s.openstack.cluster_name == current_cluster_name);
        message = ''
          Will not update terraform config because there is a mismatch between the deployed and future cluster_name. This would cause death and destruction.
          Set `openstack.cluster_name` back to ${current_cluster_name}. Your suggested change ${config.yk8s.openstack.cluster_name} is unacceptable.
        '';
      })
    ];
    _inventory_packages =
