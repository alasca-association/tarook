          ...
          (
            ...
            // rec {
              ...
              volume_name =
                ...
                then "${vm_name}-volume"
                else "${config.yk8s.infra.cluster_name}-${nodeValues.role}-volume-${nodeName}";
            }
          ))
      );
    };
  ...
  config.yk8s = lib.mkMerge [
    ...
    (lib.mkIf cfg.enabled {
      ...
      infra = {
        ...
        public_fixed_ip = config.yk8s.terraform.outputs.networking_fixed_ip.value or null;
        public_fixed_ip_v6 = config.yk8s.terraform.outputs.networking_fixed_ip_v6.value or null;
        ansible_hosts = let
