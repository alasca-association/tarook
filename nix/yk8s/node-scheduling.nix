...
in {
  options.yk8s.node-scheduling = mkTopSection {
    ...
    labels = mkOption {
      ...
      '';
      type = with types; attrsOf (either (listOf k8sLabelStr) (attrsOf (nullOr k8sLabelValue)));
      default = {};
      example = {
    ...
    taints = mkOption {
      ...
      '';
      type = with types;
        attrsOf (either (listOf k8sTaintStr) (attrsOf (nullOr (submodule {
          options = {
            value = mkOption {
              type = nullOr k8sLabelValue;
              default = null;
            };
            effect = mkOption {
