    ...
    perSystem =
      ...
      in {
        config.packages = let
          ...
          /*
          Extracts an attribute set of sections containing the doc preface and doc order
          */
          allSections =
            lib.attrsets.foldlAttrs (acc: n: v: let
          ...
          filteredNixosOptionsDoc = prefix:
            pkgs.nixosOptionsDoc {
              options = eval.options;
              documentType = "none";
              transformOptions = v:
                if lib.strings.hasPrefix "perSystem.${prefix}" v.name
                then
                  v
                  // {
          ...
          nixosOptionsDoc = filteredNixosOptionsDoc "yk8s";
          ...
          rstDocStr = section: values: let
            options = (filteredNixosOptionsDoc section).optionsNix;
            filename = "${section}.rst";
          in (''
              ${
          ...
          rstDocWithIndex = sections: let
            docs = pkgs.buildEnv {
              ...
              paths = lib.attrsets.mapAttrsToList rstDoc allSections;
            };
