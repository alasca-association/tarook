...
``MANAGED_K8S_WG_USER_GIT``             ``gitlab.cloudandheat.com:lcm/wg_user``                                 Git URL to a repository with wireguard
                                                                                                                ...
                                                                                                                (see below).
``TOFU_MODULE_PATH``                    ``../tofu``                                                             Path to the OpenTofu root module to
...
Environment Variable        Default                                     Description
=========================== =========================================== ====================
``TF_VAR_keypair``          ``"firstnamelastname-hostname-gendate"``    Defines the keypair name (in OpenStack) which will be used during the creation of new instances. Does not affect instances which have already been created. You **MUST** adjust this variable if you want to deploy on top of OpenStack. This variable is used by the ``apply-tofu.sh``:ref:`-script<actions-references.apply-tofush>`.
...
``TF_USAGE``                                ``true``    Allows to disable execution of the tofu stage by setting it to false. This is also taken into account by the inventory helper. Intended use case are bare-metal or otherwise pre-provisioned setups.
...
.. note::
   ...
   that is that OpenTofu is not configured to delete the data by
