...
.. Tip::
  ...
  It is recommended to use a local (uncommitted) [direnv `.envrc`](https://direnv.net/) to have your shell automatically set the required variables.
...
.. envvar:: MANAGED_K8S_COLOR_OUTPUT
...
.. WARNING::
  ...
  These credentials are copied into the cluster. You SHOULD use
  a dedicated OpenStack project for your cluster.

.. WARNING::

  Only use this exact set of variables. Using other, semantically
...
.. WARNING::
  ...
  Currently the combination of thanos and application credentials [is not supported](../managed-services/prometheus/prometheus-stack.md#thanos).
...
.. envvar:: MANAGED_K8S_GIT

  This git URL is used by ``init.sh`` to bootstrap the LCM (``yaook/k8s``) repository. Can be used to override the repository to use another mirror.

  Default: ``gitlab.com:yaook/k8s``

.. envvar:: MANAGED_K8S_WG_USER_GIT

  Git URL to a repository with wireguard keys to provision. Can be enabled by setting :envvar:`WG_COMPANY_USERS` (see below).

  Default: ``gitlab.cloudandheat.com:lcm/wg_user``

.. envvar:: MANAGED_K8S_PASSWORDSTORE_USER_GIT

  Git URL to a repository with users to grant access to cluster secrets. Can be enabled by setting :envvar:`PASS_COMPANY_USERS` (see below).

  Default: ``gitlab.cloudandheat.com:lcm/mk8s-passwordstore-users``

.. envvar:: MANAGED_CH_ROLE_USER_GIT

  URL to the ch-role-users role submodule. Can be enabled by setting :envvar:`SSH_COMPANY_USERS` (see below).

  Default: ``gitlab.cloudandheat.com:operations/ansible-roles/ch-role-users.git``

Secret Management
=================

.. envvar:: PASS_COMPANY_USERS

  If set to true, `init.sh` will clone the repository :envvar:`MANAGED_K8S_PASSWORDSTORE_USER_GIT`. The users in that repository will be granted access to the pass-based secret store.
...
.. envvar:: wg_conf_name
  ...
  This variable defines the name of the WireGuard interface to create. Interface name length is restricted to 15 bytes and should start with `wg`. Examples: `wg0`, `wg-k8s-dev`. This variable is used by the [`wg-up.sh`-script](../operation/actions-references.md#wg-upsh).

  Default: ``"wg0"``

.. envvar:: wg_private_key_file

  Path to your WireGuard private key file. This is not copied to any remote machine, but needed to generate the local configuration locally and to bring the VPN tunnel up. You **MUST** adjust this variable or :envvar:`wg_private_key`. This variable is used by the [`wg-up.sh`-script](../operation/actions-references.md#wg-upsh).

  Default: ``"$(pwd)/../privkey"``

.. envvar:: wg_private_key

  Alternatively you can directly export your WireGuard private key instead of a path to it. The key is injected via `wg set` to prevent leakage. This variable is used by the [`wg-up.sh`-script](../operation/actions-references.md#wg-upsh).


.. envvar:: wg_user

  Your WireGuard user name as defined in the [wireguard configuration](./cluster-configuration.md#wireguard-configuration) (or, if enabled, [`wg_user` repository](https://gitlab.cloudandheat.com/lcm/wg_user)). You **MUST** adjust this variable. This variable is used by the [`wg-up.sh`-script](../operation/actions-references.md#wg-upsh).

  Default: ``"firstnamelastname"``

.. envvar:: WG_COMPANY_USERS

  If set to true, `init.sh` will clone the repository :envvar:`MANAGED_K8S_WG_USER_GIT`. The inventory updater will then configure the wireguard users from that repository.
...
.. envvar:: MANAGED_K8S_SSH_USER

  The SSH user to use to log into the machines. This variable *SHOULD* be set. By default, the Ansible automation is written such that it’ll auto-detect one of the default SSH users (`centos`, `debian`, `ubuntu`) to connect to the machines. This only works if the machines were created with a keypair of which you hold the private key (see :envvar:`TF_VAR_keypair`). If the LCM is configured to roll out all relevant users from the [ch-users-databag](https://gitlab.cloudandheat.com/configs/ch-users-databag/) via [ch-role-users](https://gitlab.cloudandheat.com/operations/ansible-roles/ch-role-users) (see :envvar:`SSH_COMPANY_USERS`), you'll need to ensure that this the correct user is used when trying to bring up the SSH connection.

.. envvar:: SSH_COMPANY_USERS

  If set to true, `init.sh` will clone the repository :envvar:`MANAGED_CH_ROLE_USER_GIT`. The inventory updater will then configure your inventory such that the `ch-role-users` role is executed in stage2 and stage3.
...
.. envvar:: MANAGED_K8S_RELEASE_THE_KRAKEN
...
.. envvar:: MANAGED_K8S_NUKE_FROM_ORBIT
...
.. envvar:: MANAGED_K8S_IGNORE_WIREGUARD_ROUTE
...
.. envvar:: K8S_CUSTOM_STAGE_USAGE

  If set to true, `init.sh` will create a base skeleton for the [customization layer](../design/abstraction-layers.md#customization) in your cluster repository. Also the [`apply.sh`-script](./../operation/actions-references.md#applysh) will now include the appliance of this stage.

  .. Note::

    If you have already initialized you cluster repository, you'll need to rerun the [`init.sh`-script](./../operation/actions-references.md#initsh) after enabling the Customization layer.
