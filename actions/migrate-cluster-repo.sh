...
if [ -z "${K8S_CUSTOM_STAGE_USAGE:-}" ] || [ "${K8S_CUSTOM_STAGE_USAGE}" == 'false' ]; then
  ...
  if [ ! -L "$ansible_k8s_custom_playbook_dir/vars/k8s-core-vars" ]; then
    run ln -sf "../../$submodule_managed_k8s_name/k8s-core/ansible/vars/" "$ansible_k8s_custom_playbook_dir/vars/k8s-core-vars"
  fi
