    ...
    if ! harbour_disruption_allowed; then
        ...
        if [ "${tf_usage:-true}" == 'true' ]; then
            if [ -z ${terraform_prevent_disruption+x} ]; then
                errorf "and ${terraform_disruption_setting} in the config is unset or invalid" >&2
            else
                errorf "and ${terraform_disruption_setting} in the config is set to %q" \
